// simple jQuery function for buttons that toggle between two states and wait for clicker server events in between
// @author pbeshai
...
(function () {
	...
	jQuery.fn.csToggleButton = function(settings) {
		var defaults = {
			classState1: "state-1",
			classState2: "state-2",
			...
			state1to2Event: null,  // string event name from ClickerServer
			state2to1Event: null,  // string event name from ClickerServer
		};
		return this.each(function() {
			...
			if (config.state1To2Event !== null) {
		  	ClickerServer.on(config.state1To2Event, toState2);
		  }
		  if (config.state2To1Event !== null) {
		  	ClickerServer.on(config.state2To1Event, toState1);
		  }
